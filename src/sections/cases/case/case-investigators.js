import { Unstable_Grid2 as Grid, TextField, Button, Card, CardContent, CardActions, Divider, Skeleton, Autocomplete, Chip, Box, Avatar } from '@mui/material';
import { LoadingButton } from '@mui/lab';
import React, { useState, useEffect } from 'react';

const CaseInvestigators = (props) => {
    const { state, investigators, loading, loadingButtonDetails, loadingButtonPicture, handleSubmit, handleEdit, handleCancel, handleChangeInvestigators } = props;
    ...
    const handleCancelInvestigators = () => {
        setIsClicked(false);
        setIsFieldDisabled(true);
        handleCancel();
    }
    ...
    useEffect(() => {
        ...
        setValue(investigators.filter((investigator) => state.investigatorIds?.includes(investigator.id)));
    }, []);
    return (
        <Card
            sx={{
        ...
        >
            <CardContent>
                <Grid container spacing={3}>
                    {[
                        { label: 'Điều tra viên', name: 'investigatorIds', required: true, autoComplete: true },
                    ].map((field) => (
                                ...
                                field.autoComplete && (
                                    <Autocomplete
                                        ...
                                        onChange={(event, value) => {
                                            handleChangeInvestigators(value);
                                        }}
                                        defaultValue={value}
